...
{if is_array($_pez)}
    <aside class="accordion">
        {foreach $_pez as $name => $s0}
            {if strlen({$name}) > 0 }
                <h1>{$name} <a href="#" style="float: right;">{jrCore_icon icon="plus" size=20}</a></h1>
                <div>
                    {if is_array($s0.children)}
                        {foreach $s0.children as $label => $s1}
                            {if strlen({$label}) > 0}
                                <h2>{$label} <a href="#" style="float: right;">{jrCore_icon icon="plus" size=20}</a></h2>
                                <div>
                                    {if is_array($s1.children)}
                                        {foreach $s1.children as $s1_label => $s2}
                                            {if strlen({$s1_label}) > 0 }
                                                <h3>{$s1_label} <a href="#" style="float: right;">{jrCore_icon icon="plus" size=20}</a></h3>
                                            {/if}
                                        {/foreach}
                                    {/if}
                                </div>
